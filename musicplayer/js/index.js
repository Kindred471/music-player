$(function(){
    ...
    var playNextBtn = $('#player_next')               // 下一首按钮 dom元素

    var time = $('.time');                     // 时间信息部分 dom元素
    var tProgress = $('.current_time');        // 当前播放时间文本部分 dom元素
    var totalTime = $('.total_time');          // 歌曲总时长文本部分 dom元素

    var sArea = $('.bar');                  // 进度条部分
    var insTime = $('#ins_time');              // 鼠标移动至进度条上面，显示的信息部分
    var sHover = $('#s_hover');                // 鼠标移动至进度条上面，前面变暗的进度条部分
    var seekBar = $('.progress_bar');              // 播放进度条部分
    var volMuteBtn =$('.vol em');
    var volBar = $('.volBar');
    var currvol = $('.currvol');
    // 一些计算所需的变量
    ...
    function selectTrack(flag){
     ...
     function volmute(){
        if(audio.muted){
            volMuteBtn.attr('class','bi bi-volume-down-fill');
            audio.muted=false;
        }else{
            volMuteBtn.attr('class','bi bi-volume-mute-fill');
            audio.muted=true;
        }
     }
     function volbegin() {
    ...
    function initPlayer() {
       ...
       document.getElementById("volBar").addEventListener("click",function (e){
